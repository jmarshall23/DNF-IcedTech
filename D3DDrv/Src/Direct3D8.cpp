...
const char* __stdcall DXGetErrorString8(HRESULT hr) {
	return "todo";
}
...
// jmarshall - when you upload to a buffer thats current rendering you stall the render pipeline,
// this code will queue up buffers for rendering which will prevent stalls. 
#define NUM_VERT_POOL_BUFFFERS					5
template <class T>
class FD3DVertexBuffer
{
public:
	IDirect3DDevice8* Device;
	IDirect3DVertexBuffer8* VertexBuffer8[NUM_VERT_POOL_BUFFFERS];
	INT						Length,
		First,
		Rover;

	int						currentPoolBufferId;
	...
	FD3DVertexBuffer()
	{
		...
		Length = 0;
		Rover = 0;
		VertexBuffer8[0] = NULL;
		VertexBuffer8[1] = NULL;
		Device = NULL;
		currentPoolBufferId = 0;
	}
	...
	void Init(IDirect3DDevice8* InDevice, INT InLength)
	{
		...
		for (int i = 0; i < NUM_VERT_POOL_BUFFFERS; i++)
		{
	...
	{
		for(int i = 0; i < NUM_VERT_POOL_BUFFFERS; i++)
			SafeRelease(VertexBuffer8[i]);		
	}

	void Swap()
	{
		currentPoolBufferId++;
		if (currentPoolBufferId >= NUM_VERT_POOL_BUFFFERS)
			currentPoolBufferId = 0;
	}
	...
	{
		check(VertexBuffer8[currentPoolBufferId]);
		check(Length);
		check(Num < Length);
		...
		if (Start != -1)
		{
			D3D_CHECK(VertexBuffer8[currentPoolBufferId]->Lock(Start * sizeof(T), Num * sizeof(T), (BYTE**)&VertexData, D3DLOCK_NOOVERWRITE));
		}
		else if (Rover + Num < Length)
		{
			D3D_CHECK(VertexBuffer8[currentPoolBufferId]->Lock(Rover * sizeof(T), Num * sizeof(T), (BYTE**)&VertexData, D3DLOCK_NOOVERWRITE));
			First = Rover;
		...
		{
			D3D_CHECK(VertexBuffer8[currentPoolBufferId]->Lock(0, Num * sizeof(T), (BYTE**)&VertexData, D3DLOCK_DISCARD));
			First = 0;
			Rover = Num;
		}
	...
	INT Unlock()
	{
		...
		D3D_CHECK(VertexBuffer8[currentPoolBufferId]->Unlock());
	...
	//	D3D_CHECK(Device->SetRenderState((D3DRENDERSTATETYPE)D3DRS_SOFTWAREVERTEXPROCESSING, FALSE));
		D3D_CHECK(Device->SetVertexShader(T::FVF));
		D3D_CHECK(Device->SetStreamSource(0, VertexBuffer8[currentPoolBufferId], sizeof(T)));
...
class DLL_EXPORT UD3DRenderDevice : public URenderDevice
{
	...
	float m_RFX2, m_RFY2;
	float m_Aspect, m_RProjZ;
	float m_sceneNodeX, m_sceneNodeY;
...
// jmarshall end
	...
	void __fastcall QueuePolygonEnd(DWORD ProjectorFlags = 0)
	{
		...
		if ((CurrentFrame->X != m_sceneNodeX) || (CurrentFrame->Y != m_sceneNodeY)) {
			//m_sceneNodeHackCount++;
...
// jmarshall
		...
		if (selHit || bForce) {
			...
			if (nHitNameBytes <= m_HitBufSize) {
				m_gclip.GetHitNameStackValues((unsigned int*)HitData, nHitNameBytes / 4);
				HitCount = nHitNameBytes;
			}
			else {
				HitCount = 0;
			}
...
// jmarshall end
	...
	float __fastcall SetZBias( FLOAT NewZBias=0.f )
	{
		...
		{
			ZBias=NewZBias;
			Direct3DDevice8->SetRenderState((D3DRENDERSTATETYPE)D3DRS_ZBIAS,ZBias);
		}
	...
	UBOOL __fastcall SetRes( INT NewX, INT NewY, INT NewColorBytes, UBOOL Fullscreen )
	{
		...
		D3DPRESENT_PARAMETERS8 PresentParms;
