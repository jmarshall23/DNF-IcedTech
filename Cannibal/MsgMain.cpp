...
NBool msg_call_stack(IMsg* msg, IMsgTarget* target, CC8* pstr, U32 argc, IMsgToken* token, void* func) {
	NBool(*entry)(IMsgTarget * target, IMsg * msg, ...);

	entry = (NBool(__cdecl*)(IMsgTarget*, IMsg *, ...))func;

	switch (argc - 1)
	{
		case 0:
			return entry(target, msg);

		case 1:
			return entry(target,msg, token[0].GetType());

		case 2:
			return entry(target, msg, token[0].GetType(), token[1].GetType());

		case 3:
			return entry(target, msg, token[0].GetType(), token[1].GetType(), token[2].GetType());
		case 4:
			return entry(target, msg, token[0].GetType(), token[1].GetType(), token[2].GetType(), token[3].GetType());

		case 5:
			return entry(target, msg, token[0].GetType(), token[1].GetType(), token[2].GetType(), token[4].GetType(), token[5].GetType());
	}
...
// jmarshall - was - we walk through the list forward to back in the new c++ code. 
				// return msg_call_stack(inMsg, inTarget, pstr, inMsg->m_Argc, &inMsg->m_Argv[inMsg->m_Argc - 1], impFunc);
				return msg_call_stack(inMsg, inTarget, pstr, inMsg->m_Argc, &inMsg->m_Argv[1], impFunc);
// jmarshall end
			}
			return(0);
		}
